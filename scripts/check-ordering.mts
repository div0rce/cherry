...
const PREFIX = 'check:ordering';
const FIX = 'Make ordering explicit or add allowlist markers where permitted.';
  ...
  if (rawContent.includes(PRAGMA_FILE)) {
    if (isAllowlisted) {
      process.stdout.write(
        `WARNING: ${normalizePath(filePath)} is fully exempted from implicit-ordering checks\n`
      );
      return [];
    }
  }
...
function main(): void {
  ...
  if (violations.length > 0) {
    ...
    fail(PREFIX, 'Implicit ordering violations detected', { details, fix: FIX });
  }

  process.stdout.write(
    "✅ Implicit ordering check passed: no forbidden patterns in app/ or lib/\n"
  );
}
