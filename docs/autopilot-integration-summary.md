...
- **State semantics:** `state = warning` when preview status is non-OK, warnings exist, or bucket remaining ≤ 0; safety badges and risk banners are derived from that flag. Reward strength is derived from expected benefit ratio; impact segments always padded to 3 entries.
