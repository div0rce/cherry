...
- Total registered guardrails: 55 (from `scripts/guardrails/registry.mts`).
- Enforced in CI: 55 (via `ci:verify` → `check` → `check:guardrails`; coverage enforced by `check:ci-guardrail-coverage`).
